...
def generate_sentence(fn):

	sentence_number=0
	sentences = []
	current_sentence = []

	with open (fn, "r") as sen_file:

		for i, line in enumerate(sen_file):
			if line =="\n":
			...
			if line !="\n":
				fields = line.split("\t")
				word = fields[1]
				current_sentence.append(sanitize_word(word))
...
def generate_terminal_word(conll, grammar_fn):
    TEMPLATE = (
        '{0} -> {0}_{1}\n[string] {1}\n[ud] "({1}<root> / {1})"\n')

    for w_id in conll:
        print(TEMPLATE.format(sanitize_word(conll[w_id][3]), sanitize_word(conll[w_id][0])), file=grammar_fn)
